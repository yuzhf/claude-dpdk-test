...
static int create_mbuf_pool(void)
{
    ...
    /* 根据端口和队列数量调整mbuf数量 */
    for (int i = 0; i < g_app_config->n_ports; i++) {
        nb_mbufs += (g_app_config->n_rx_queues[i] + g_app_config->n_tx_queues[i]) * 2048;
    }
    ...
    g_app_config->mbuf_pool = rte_pktmbuf_pool_create("mbuf_pool", nb_mbufs,
                                                      MBUF_CACHE_SIZE, 0,
                                                      RTE_MBUF_DEFAULT_BUF_SIZE,
                                                      rte_socket_id());
    
    if (g_app_config->mbuf_pool == NULL) {
        RTE_LOG(ERR, DPDK_INIT, "Cannot create mbuf pool\n");
        return -1;
    }
    
    RTE_LOG(INFO, DPDK_INIT, "Mbuf pool created successfully\n");
    return 0;
}
...
void dpdk_cleanup(void)
{
    ...
    /* 清理内存池 */
    if (g_app_config->mbuf_pool) {
        RTE_LOG(INFO, DPDK_INIT, "Cleaning up mbuf pool\n");
        // mbuf池由EAL自动清理
        g_app_config->mbuf_pool = NULL;
    }
