...
struct clickhouse_client {
    ...
    uint32_t batch_size;                    /* 当前批量大小 */
    bool realtime_mode;                     /* 实时模式标志 */
...
/* 设置ClickHouse实时模式 */
void clickhouse_set_realtime_mode(bool enable)
{
    if (g_ch_client == NULL) {
        return;
    }

    g_ch_client->realtime_mode = enable;
    g_ch_client->batch_size = enable ? CH_REALTIME_SIZE : CH_BATCH_SIZE;

    RTE_LOG(INFO, CLICKHOUSE, "ClickHouse mode: %s (batch_size: %u)\n",
            enable ? "realtime" : "batch", g_ch_client->batch_size);
}

/* 检查是否需要定时刷新 */
static bool should_flush_by_time(void)
{
    ...
    uint64_t elapsed_sec = (current_time - g_ch_client->last_flush_time) / rte_get_timer_hz();

    return elapsed_sec >= CH_FLUSH_INTERVAL;
}
...
int clickhouse_init(void)
{
    ...
    /* 设置默认配置 */
    g_ch_client->realtime_mode = true;  /* 默认启用实时模式 */
    g_ch_client->batch_size = CH_REALTIME_SIZE;
    g_ch_client->last_flush_time = rte_get_timer_cycles();
...
int clickhouse_export_flow(struct flow_info *flow)
{
    ...
    if (g_ch_client->buffer_count < CH_BATCH_SIZE) {
        ...
        /* 检查是否需要导出：达到批量大小或定时刷新 */
        if (g_ch_client->buffer_count >= g_ch_client->batch_size ||
...
int clickhouse_export_batch(struct flow_info **flows, int count)
{
    ...
    g_ch_client->last_flush_time = rte_get_timer_cycles();  /* 更新刷新时间 */
