...
/* 全局RX统计信息 */
static struct rx_core_stats rx_stats[MAX_RX_CORES] __rte_cache_aligned;
                              ...
                              struct lcore_conf *lconf, uint16_t rx_core_idx)
{
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if (likely(worker_idx < lconf->n_worker_rings && worker_counts[worker_idx] < BURST_SIZE)) {
            ...
            /* 选择失败，使用轮询方式 */
            worker_idx = stats->tx_packets % lconf->n_worker_rings;
...
void print_rx_cores_summary(void)
{
    ...
    uint64_t total_tx_pkts = 0, total_dropped = 0;
    uint16_t i;
    
    printf("\n=== RX Cores Summary ===\n");
    printf("%-8s %-12s %-12s %-12s %-12s %-12s\n",
           "Core", "RX_Packets", "RX_Bytes", "TX_Packets", "Dropped", "Ring_Drops");
    printf("%-8s %-12s %-12s %-12s %-12s %-12s\n",
           "----", "----------", "--------", "----------", "-------", "----------");
    ...
    for (i = 0; i < g_app_config->n_rx_cores; i++) {
        ...
        printf("%-8u %-12" PRIu64 " %-12" PRIu64 " %-12" PRIu64 " %-12" PRIu64 " %-12" PRIu64 "\n",
               lcore_id, stats->rx_packets, stats->rx_bytes, stats->tx_packets,
               stats->dropped_packets, stats->ring_full_drops);
    ...
    printf("%-8s %-12" PRIu64 " %-12" PRIu64 " %-12" PRIu64 " %-12" PRIu64 " %-12s\n",
           "Total", total_rx_pkts, total_rx_bytes, total_tx_pkts, total_dropped, "-");
    printf("========================\n\n");
}