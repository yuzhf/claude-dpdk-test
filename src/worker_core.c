...
/* 全局Worker统计信息 */
static struct worker_core_stats worker_stats[MAX_WORKER_CORES] __attribute__((__aligned__(RTE_CACHE_LINE_SIZE)));
...
static void print_worker_stats(uint16_t worker_idx, uint16_t lcore_id)
{
    ...
    if (elapsed_seconds >= 8.0) {
        ...
        RTE_LOG(INFO, WORKER,
                "Worker Core %u: %" PRIu64 " pkts, "
                "%" PRIu64 " bytes, "
                "%" PRIu64 " processed, %" PRIu64 " new_flows, "
                "%" PRIu64 " dropped\n",
                lcore_id, stats->rx_packets, stats->rx_bytes,
